...
class myBattery(Architecture):
    def __init__(self, arch_type, left, top):
        super(myBattery, self).__init__()
        # 设置位置坐标
        self.x = left
        self.y = top
        # 角度，逆时针，竖直向上为0度
    ...
    def shot(self):
        ...
        if now_time - self.time >= self.next_fire:
            ...
            # 确定下次开炮时间
            self.next_fire = self.fire_rate * (random.random() * 0.6 + 0.7)
            return MyBatteryBullet(self, self.bullet)
...
class MyBatteryBullet(gamebullet.Bullet):
    def __init__(self, arch: myBattery, bullet_type):
        super(MyBatteryBullet, self).__init__()
        f = None
        if bullet_type == 'HE203':
            f = open('entity/bullet/HE203.json')
        if f is not None:
